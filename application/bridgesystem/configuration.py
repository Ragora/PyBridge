...
class GlobalConfiguration(ConfigurationBase):
    ...
    class ProcessInternal(ConfigurationBase):
        def __init__(self, configuration={}):
            self.debug = ConfigurationBase.ConfigurationValue(name="debug", default=False, value_type=bool)
        ...
        debug = None
        """
            Whether or not debug prints are enabled.
        """
        ...
        force_stop = None
        """
            If an internal error occurs, should the process use sys.exit(1) to force an exit instead of trying to
            gracefully exit.
        """
